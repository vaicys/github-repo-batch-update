...
$resp = Invoke-RestMethod -Headers $headers -Uri "https://api.github.com/search/repositories?q=$searchPattern+org:$org+archived:false"
$repos = $resp.items | Where { $_.name -Match "^$searchPattern" } | ForEach { $_.url }

# Confirm
Write-Host "The following repositories will be updated:"
$repos | ForEach { Write-Host $_ }
